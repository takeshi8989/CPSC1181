...
public class GoGameClient extends Application {
	...
	private GoGameClientService service;
	...
	public void start(Stage primaryStage) {
		service = new GoGameClientService();
	...
	private static class GoGameClientService implements Runnable {
		final int PORT = 1181;
		...
		public void run() {
			try (Socket s = new Socket("localhost", PORT)) {
				...
				while(true) {
					...
					switch(commandType) {
						case "PLAYER": 
							setServerPlayerName(command[1]);
							break;
						case "PLAY":
							serverPlay(command[1], command[2]);
					}
				}
			} catch (IOException e){
				e.printStackTrace();
		...
		public void setClientPlayerName() throws IOException {
		...
		public void clientPlay(MouseEvent e) throws IOException {
			double x = e.getX();
			double y = e.getY();
			boolean emtpyCell = putStone(x, y, 1);
		...
		public void serverPlay(String mouseX, String mouseY) {
			putStone(Double.parseDouble(mouseX), Double.parseDouble(mouseY), 0);
			yourTurn = true;
			gameStart = true;
		}
